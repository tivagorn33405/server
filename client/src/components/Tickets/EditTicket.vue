<template>
  <div class="container ticket-wrapper">
    ...
    <h1>Edit Ticket</h1>
    <form v-on:submit.prevent = "editTicket" >
      <!-- <p>title: <input type="text" v-model="ticket.title"></p> -->
      <p>
        <label for="" class="control-label">Title: </label>
        <input type="text" v-model="ticket.title" class="form-control">        
      </p>
      <transition name="fade">
        <div class="thumbnail-pic" v-if="ticket.thumbnail != 'null'">
          <img class="img-thumbnail" :src="BASE_URL+ticket.thumbnail" alt="thumbnail">
        </div>
      </transition>  
      ...
      <p><vue-ckeditor v-model.lazy="ticket.content" :config="config" @blur="onBlur($event)" @focus="onFocus($event)" /></p>            
      <p>
        <label class="control-label">Category :</label>
        <input type="text" v-model="ticket.category" class="form-control">
      </p>     
      <p>
        <label class="control-label">Prices :</label>
        <input type="text" v-model="ticket.prices" class="form-control">
      </p>       
      <p>
        <button class="btn btn-warning" type="submit">Update Ticket</button>
        <button class="btn btn-default" type="button" v-on:click="navigateTo('/tickets')">Back</button>
      </p>      
...
export default {
  ...
  methods: {
    ...
    // ticket
    async editTicket () {
      this.ticket.pictures = JSON.stringify(this.pictures)
      try {
        await TicketsService.put(this.ticket)
        this.$router.push({
          name: 'tickets'
        })
      } catch (err) {
    ...
    useThumbnail (filename) {     
      ...
      this.ticket.thumbnail = filename
    },
  ...
  async mounted () {
    ...
    try {      
      ...
      this.pictures = JSON.parse(this.ticket.pictures)
    } catch (error) {
      console.log (error)
    }
  },
